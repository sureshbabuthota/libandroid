/*
 * @author Srikar created on @date Aug 12, 2014
 *
...
/**
 * A command APDU as per the structure defined in ISO/IEC 7816-4. It consists
 * of a four byte header and a conditional body of variable length. This class
 * does not verify that the APDU encodes a semantically valid command.
 *
 *<p>
 * Note that when the expected length of the response APDU is specified in the
 * {@linkplain #CommandAPDU(int,int,int,int,int) constructors}, the actual
 * length (Ne) must be specified, not its encoded form (Le). Similarly,
 * {@linkplain #getNe} returns the actual value Ne. In other words, a value of 0
 * means "no data in the response APDU" rather than "maximum length."
 *</p>
 *
 *<p>
 * This class supports both the short and extended forms of length encoding for
 * Ne and Nc. However, note that not all terminals and Smart Cards are capable
 * of accepting APDUs that use the extended form.
 *</p>
 *
 *<p>
 * For the header bytes CLA, INS, P1, and P2 the Java type <code>int</code> is
 * used to represent the 8 bit unsigned values. In the constructors, only the 8
 * lowest bits of the <code>int</code> value specified by the application are
 * significant. The accessory methods always return the byte as an unsigned value
 * between 0 and 255.
 *</p>
 *
 *<p>
 * Instances of this class are immutable. Where data is passed in or out via
 * byte arrays, defensive cloning is performed.
 *</p>
 * @see ResponseAPDU
 * 
 */
public final class CommandAPDU implements java.io.Serializable {
	...
	/**
	 * Constructs a CommandAPDU from a byte array containing the complete APDU
	 * contents (header and body). The APDU starts at the index
	 ...
	 *             if apdu is null
	 * @throws IllegalArgumentException
	 *             if apduOffset or apduLength are negative or if apduOffset +
	 *             apduLength are greater than apdu.length, or if the specified
	 ...
	 * contents (header and body). The buffer's <code>position</code> must be
	 * set to the start of the APDU, its <code>limit</code> to the end of the
	 * APDU. Upon return, the buffer's <code>position</code> is equal to its
	 * limit; its limit remains unchanged.
	 *
	 * <p>
	...
	/**
	 * Constructs a CommandAPDU for select applet given applet ID .
	 *
	 * <br>
	 * <br>
	 * <b>Note:</b>Header for select applet command for any applet is constant
	 * as follows. <br>
	 * |CLA|INS|P1 |P2 | <br>
	 * |0x00| 0xA4|0x04|0x00|
	 * 
	 * @param appletID
	 *            Hex String value off applet ID.
	 * 
	 * 
	 * @throws IllegalArgumentException
	 *             if applet ID is not a valid Hex String.
	 */

	public CommandAPDU(String appletID) {
		// 00A40400
		this(0x00, 0xA4, 0x04, 0x00, ConversionHelper
				.hexStringToByteArray(appletID), 0x00);

	}

	/**
	 * Constructs a CommandAPDU from the four header bytes. This is case 1 in
	 * ISO 7816, no command body.
	 *
	 * @param cla
	 *            the class byte CLA
	 * @param ins
	 ...
	 * 
	 * <br>
	 *            <b>Note: </b> Value for cla,ins,p1 and p2 should be of one
	 *            byte each.
	 */
	 ...
	 * field with Ne specified. If Ne is 0, the APDU is encoded as ISO 7816 case
	 * 1.
	 *
	 * @param cla
	 *            the class byte CLA
	 ...
	 *            the parameter byte P2
	 * @param ne
	 *            the maximum number of expected data bytes in a response APDU <br>
	 *            <b>Note: </b> Value for cla,ins,p1 and p2 should be of one
	 *            byte each.
	 *
	 * @throws IllegalArgumentException
	 ...
	 * This is case 3 in ISO 7816, command data present and Ne absent. The value
	 * Nc is taken as data.length. If <code>data</code> is null or its length is
	 * 0, the APDU is encoded as ISO 7816 case 1.
	 *
	 * <p>
	 ...
	 * Constructs a CommandAPDU from the four header bytes and command data.
	 * This is case 3 in ISO 7816, command data present and Ne absent. The value
	 * Nc is taken as dataLength. If <code>dataLength</code> is 0, the APDU is
	 * encoded as ISO 7816 case 1.
	 *
	 * <p>
	 * Note that the data bytes are copied to protect against subsequent
	 * modification.
	 ...
	 *            the byte array containing the data bytes of the command body
	 * @param dataOffset
	 *            the offset in the byte array at which the data bytes of the
	 *            command body begin
	 * @param dataLength
	 ...
	 * @throws IllegalArgumentException
	 *             if dataOffset or dataLength are negative or if dataOffset +
	 *             dataLength are greater than data.length or if dataLength is
	 *             greater than 65535
	 */
	public CommandAPDU(int cla, int ins, int p1, int p2, byte[] data,
	...
	/**
	 * Constructs a CommandAPDU from the four header bytes, command data, and
	 * expected response data length. This is case 4 in ISO 7816, command data
	 * and Ne present. The value Nc is taken as data.length if <code>data</code>
	 * is non-null and as 0 otherwise. If Ne or Nc are zero, the APDU is encoded
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             if data.length is greater than 65535 or if ne is negative or
	 *             greater than 65536
	 */
	 ...
	 * Constructs a CommandAPDU from the four header bytes, command data, and
	 * expected response data length. This is case 4 in ISO 7816, command data
	 * and Le present. The value Nc is taken as <code>dataLength</code>. If Ne
	 * or Nc are zero, the APDU is encoded as case 1, 2, or 3 per ISO 7816.
	 *
	 * <p>
	 * Note that the data bytes are copied to protect against subsequent
	 * modification.
	 ...
	 * @throws IllegalArgumentException
	 *             if dataOffset or dataLength are negative or if dataOffset +
	 *             dataLength are greater than data.length, or if ne is negative
	 *             or greater than 65536, or if dataLength is greater than 65535
	 */
	public CommandAPDU(int cla, int ins, int p1, int p2, byte[] data,
	...
	/**
	 * Creates header part of a CommandAPDU. Header |CLA|INS|P1 |P2 |
	 * 
	 * @param cla
	 *            CLA of this APDU.
	 * @param ins
	...
	/**
	 * Returns a Hex string representation of this command APDU.
	 *
	 * @return a Hex String representation of this command APDU.
	 */
	public String toString() {
		return ConversionHelper.byteArrayToHex(apdu);
	}
